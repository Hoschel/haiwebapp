...
const BuilderPage = () => {
  ...
  const handlePublish = async () => {
    if (!id || saveState === "conflict" || saveState === "saving") { if (saveState === "conflict") toast.error("Resolve the save conflict before publishing."); return; }
    setPublishing(true);
    try { await api.post(`/api/project/${id}/publish`); const url = `${window.location.origin}/publish/${id}`; setPublishUrl(url); toast.success("Website published successfully!"); }
    catch (err) { console.error("Publish failed:", err); toast.error(err?.response?.data?.error || "Publish failed"); }
    finally { setPublishing(false); }
  };
  const handleDownload = () => { if (activeProject) exportProjectZip(activeProject); };

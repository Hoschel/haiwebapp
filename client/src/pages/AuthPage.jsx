...
const AuthPage = ({mode}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || (mode === "login" ? "Invalid email or password" : "Registration failed. Please try again."));
    } finally {
      setLoading(false)
    }
  };
        ...
        <div className="w-full max-w-sm">
          <div className="mb-10">
            ...
            <p className="text-sm text-zinc-400">
              {isLogin ? 'Enter your credentials to access your website builder.' : 'Get started by entering your registiration details.'}
            </p>
          </div>
        ...
        <form className='space-y-6' onSubmit={handleSubmit}>
          {!isLogin && (
            <div>
              ...
              <input type="text" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} required className="w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 transition-colors" />
            </div>
          )}
          ...
          <div>
              ...
              <input type="email" placeholder="john@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 transition-colors" />
            </div>
          ...
          <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full pl-2 py-2 border-b border-zinc-200 focus:outline-none focus:border-zinc-950 text-sm text-zinc-900 bg-transparent placeholder-zinc-300 pr-8" />
